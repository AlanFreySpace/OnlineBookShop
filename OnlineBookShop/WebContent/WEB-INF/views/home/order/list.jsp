							...
							<td colspan="5">
								<table class="good" style="font-size:12px;">
										...
										<tr>
											...
											<th style="text-align:center;color:red;">
												<c:if test="${order.status==0 }">
													待发货
													<a href="javascript:void(0)" data-id="${order.id }" class="cancel-order-btn">[我想退货]</a>
												</c:if>
									...
									<tbody>
										...
										<tr><td colspan="6" class="dingdan_title">订单备注：${order.remark }&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;收货地址:${order.address }</td></tr>
			...
			$(".cancel-order-btn").click(function(){
				...
				if(confirm("确定退货?")){
					$.ajax({
						...
						success:function(data){
							if(data.type=='success'){
								alert('您的退货申请已经提交，待商家同意后退款会自动打入您的账户中');
								window.location.reload();
							}else{
								alert(data.msg);
							}
